...
import org.apache.iceberg.{DataFile, FindFiles}
import org.apache.iceberg.expressions.Expressions
import org.apache.iceberg.spark.{SparkFilters, SparkSchemaUtil}
import org.apache.iceberg.spark.source.{SparkIcebergTable, StagingTableHelper}
import org.apache.spark.sql.{Column, DataFrame, Dataset, Row, SparkSession}
import org.apache.spark.sql.catalyst.InternalRow
import org.apache.spark.sql.catalyst.encoders.{ExpressionEncoder, RowEncoder}
import org.apache.spark.sql.catalyst.expressions.{Attribute, Expression, Literal, NamedExpression,
  PredicateHelper, UnsafeProjection}
...
case class MergeIntoCommand(target: SparkIcebergTable,
                            source: DataFrame,
  ...
  with PredicateHelper with AnalysisHelper {
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    val txn = target.getIcebergTable.newTransaction()
    ...
    val joinDF = {
      ...
      val targetDF = StagingTableHelper(target).buildDataFrame(filesToRewrite)
    ...
    def resolveOnJoinedPlan(exprs: Seq[Expression]): Seq[Expression] = {
      exprs.map { expr => tryResolveReferences(target.sparkSession())(expr, joinedPlan) }
    }
    ...
    val outputDF =
      Dataset.ofRows(target.sparkSession(), joinedPlan).mapPartitions(processor.processPartition)(outputRowEncoder)

    val files = StagingTableHelper(target).writeTableAndGetFiles(outputDF).toList
  ...
  def findFilesToRewrite(source: Dataset[Row], condition: Expression): Seq[DataFile] = {
    val targetOnlyCondition = projectPredication(target.plan(), condition)
    val filter = if (targetOnlyCondition == null || targetOnlyCondition.isEmpty) {
      Expressions.alwaysTrue()
    } else {
      targetOnlyCondition.flatMap{ cond =>
        toFilter(resolve(target.sparkSession(), target.plan(), cond))
      }.map(SparkFilters.convert).reduceLeft(Expressions.and)
    }

    val partialFilteredFiles = FindFiles.in(target.getIcebergTable)
    ...
    target.sparkSession().sparkContext.register(touchedFilesAccum, "MergeInto.touchedFiles")
    ...
    val joinDF = {
      val targetDF = StagingTableHelper(target).buildDataFrame(partialFilteredFiles)
...
class JoinedRowProcessor(targetRowHasNoMatch: Expression,
