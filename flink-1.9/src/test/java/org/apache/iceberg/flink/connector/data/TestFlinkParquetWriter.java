...
public class TestFlinkParquetWriter extends DataTest {
  ...
  private void writeAndValidate(Iterable<Row> iterable, Schema schema) throws IOException {
    File testFile = temp.newFile();
    Assert.assertTrue("Delete should succeed", testFile.delete());

    LogicalType logicalType = FlinkSchemaUtil.convert(schema.asStruct());
  ...
  protected void writeAndValidate(Schema schema) throws IOException {
    writeAndValidate(RandomData.generate(schema, NUM_RECORDS, 19981), schema);

    writeAndValidate(RandomData.generateDictionaryEncodableData(schema, NUM_RECORDS, 21124), schema);

    writeAndValidate(RandomData.generateFallbackData(schema, NUM_RECORDS, 21124, NUM_RECORDS / 20),
        schema);
  }
}
