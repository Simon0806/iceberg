...
abstract class AbstractRewriteOperator<InT, OutT> extends AbstractStreamOperator<OutT>
  ...
  AbstractRewriteOperator(Configuration config) {
    this.config = config;
    this.caseSensitive = config.getBoolean(IcebergConnectorConstant.CASE_SENSITIVE,
  ...
  public void open() throws Exception {
    super.open();
    this.table = IcebergTableUtil.findTable(config);
  ...
  protected List<DataFile> generateCurrentDataFiles() {
    return generateFilteredGroupedTasks().values().stream()
