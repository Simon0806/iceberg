...
public class FlinkOrcReader implements OrcRowReader<Row> {
  ...
  public static OrcRowReader<Row> buildReader(Schema schema, TypeDescription readSchema) {
  ...
  private static class ReadBuilder extends OrcSchemaWithTypeVisitor<OrcValueReader<?>> {
    ...
    @Override
    public OrcValueReader<Row> record(Types.StructType iStruct, TypeDescription record, List<String> names,
                                          List<OrcValueReader<?>> fields) {
      return FlinkOrcReaders.struct(fields, iStruct, idToConstant);
    }

    @Override
    public OrcValueReader<Object[]> list(Types.ListType iList, TypeDescription array,
                                          OrcValueReader<?> elementReader) {
      return FlinkOrcReaders.array(elementReader);
    }

    @Override
    public OrcValueReader<Map<?, ?>> map(Types.MapType iMap, TypeDescription map,
