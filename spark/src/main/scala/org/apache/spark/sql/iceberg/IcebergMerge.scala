...
/**
 * Trait that represents a WHEN clause in MERGE. See [[IcebergMergeInto]]. It extends [[Expression]]
...
/** Trait that represents WHEN MATCHED clause in MERGE. See [[IcebergMergeInto]]. */
sealed trait IcebergMergeIntoMatchedClause extends IcebergMergeIntoClause

/** Represents the clause WHEN MATCHED THEN UPDATE in MERGE. See [[IcebergMergeInto]]. */
...
/** Represents the clause WHEN MATCHED THEN DELETE in MERGE. See [[IcebergMergeInto]]. */
...
/** Represents the clause WHEN NOT MATCHED THEN INSERT in MERGE. See [[IcebergMergeInto]]. */
...
case class IcebergMergeInto(targetTable: SparkIcebergTable,
                            sourceDF: DataFrame,
  ...
  private lazy val target = targetTable.toDF.queryExecution.analyzed
...
object IcebergMergeInto {
  def apply(targetTable: SparkIcebergTable,
            sourceDF: DataFrame,
            condition: Expression,
            whenClauses: Seq[IcebergMergeIntoClause]): IcebergMergeInto = {
    ...
    IcebergMergeInto(targetTable, sourceDF, condition,
  ...
  def resolveReferences(merge: IcebergMergeInto)
                       (resolveExpr: (Expression, LogicalPlan) => Expression): IcebergMergeInto = {
    val IcebergMergeInto(targetTable, sourceDF, condition, matchedClauses, notMatchedClause) = merge
    ...
    IcebergMergeInto(targetTable, sourceDF, resolvedCond, resolvedMatchedClauses, resolvedNotMatchedClause)
  }
