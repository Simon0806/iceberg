...
import org.apache.spark.sql.catalyst.expressions.{Alias, Attribute, AttributeReference, BindReferences, Expression,
  If, PredicateHelper, SubqueryExpression}
import org.apache.spark.sql.catalyst.plans.logical.LogicalPlan
import org.apache.spark.sql.execution.datasources.DataSourceStrategy
import org.apache.spark.sql.sources.Filter
import org.apache.spark.sql.types.StructType

trait AnalysisHelper3 extends PredicateHelper with UpdateExpressionsSupport3 {
  import AnalysisHelper3._
  ...
  def resolve(sparkSession: SparkSession,
    ...
    val partialResolvedAttr = expr.transform {
      ...
      case attr: AttributeReference =>
        // if the expression is resolved, we extract the name and re-resolve it.
        plan.resolve(attr.name.split('.'), sparkSession.sessionState.analyzer.resolver) match {
          case Some(resolvedAttr) => resolvedAttr
          case None => throw new IllegalArgumentException(s"Could not resolve $attr using columns: ${plan.output}")
        }
    }
  ...
  def toFilter(expr : Expression) : Array[Filter] = {
    ...
    splitConjunctivePredicates(expr).map{ expr =>
      val filter = DataSourceStrategy.translateFilter(expr, supportNestedPredicatePushdown = false)
      if (filter.isEmpty) {
  ...
  def buildDataFrame(spark: SparkSession, logicalPlan: LogicalPlan) : Dataset[Row] = {
    Dataset.ofRows(spark, logicalPlan)
  }
}

object AnalysisHelper3 {
