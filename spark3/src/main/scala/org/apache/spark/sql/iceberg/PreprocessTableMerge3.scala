...
case class PreprocessTableMerge3(conf: SQLConf) extends UpdateExpressionsSupport3 {

  def preProcessMatched(mergeInto: IcebergMergeInto3) : Seq[IcebergMergeIntoMatchedClause] = {
    val IcebergMergeInto3(_, _, target, _, _, _, matched, _) = mergeInto
    ...
    val processedMatched = matched.map {
      case m: IcebergMergeIntoUpdateClause =>
        ...
        // Use the helper methods for in UpdateExpressionsSupport.scala to generate expressions such
  ...
  def preProcessNotMatched(mergeInto: IcebergMergeInto3) : Option[IcebergMergeIntoInsertClause] = {
    val IcebergMergeInto3(_, _, target, _, _, _, matched, notMatched) = mergeInto
  ...
  def apply(mergeInto: IcebergMergeInto3): MergeIntoCommand3 = {
    val IcebergMergeInto3(target, sparkSession, plan, caseSensitive, source, condition, _, _) = mergeInto
    MergeIntoCommand3(target, sparkSession, plan, caseSensitive, source, condition,
      preProcessMatched(mergeInto), preProcessNotMatched(mergeInto))
  }
