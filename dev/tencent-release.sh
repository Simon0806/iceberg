...
git archive $release_hash --prefix $tag/ -o $tarball .baseline api arrow common core data dev examples flink flink-runtime gradle hive orc parquet pig project python site spark spark-runtime CHANGES DISCLAIMER LICENSE NOTICE README.md .travis.yml baseline.gradle build.gradle deploy.gradle deploy-tencent.gradle gradle.properties gradlew jitpack.yml jmh.gradle settings.gradle tasks.gradle versions.lock versions.props version.txt
